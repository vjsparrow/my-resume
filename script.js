...
let quote_of_the_day = document.getElementsByClassName("quote")[0];

// var quote_header = document.createElement("h4");
// quote_header.innerHTML = "Quote of the day";
// quote_of_the_day.appendChild(quote_header);

// let quote_content = document.createElement('p');
// quote_content.innerHTML = quotes[0]['quote'];
// quote_of_the_day.appendChild(quote_content);



let quote_content = document.createElement('h5');
quote_content.innerHTML = `"${quotes[0]["quote"]}"`;
quote_of_the_day.appendChild(quote_content);

var counter = 1;
let new_count = (quotes, quote_content, quote_of_the_day) => {
  quote_content.innerHTML = `"${quotes[counter]["quote"]}"`;
  quote_of_the_day.appendChild(quote_content);
...
let set_date = (() => {
    ...
    date_block.innerHTML = `As on ${date.toDateString().slice(0, 3)}, ${date.toDateString().slice(4, 15)}.`;
});
set_date();
setInterval(set_date, 5*60000);